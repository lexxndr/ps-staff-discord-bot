...
from g4f.Provider import RetryProvider, PollinationsAI, Dynaspark, Liaobots
from disnake.ext import commands

client = AsyncClient(provider=RetryProvider([PollinationsAI, Dynaspark, Liaobots], shuffle=False))
        ...
        if any([triggerWord in message.content.lower() for triggerWord in triggerWords]):
            response = await client.chat.completions.create(
                ...
                messages=[
                        ...
                        {"role": "user", "content": f"reply to a message by the user {message.author.name} - {message.content}"}
                    ],
                web_search=False
            )
